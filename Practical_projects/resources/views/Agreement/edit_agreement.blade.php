{{-- File: resources/views/agreements/edit.blade.php --}}
...
<head>
    ...
    <style>
        /* Custom styles to match the original Blade components */
        body {
        ...
        /* Tom Select styles to match the current design */
        .ts-wrapper {
            width: 100%;
        }
        .ts-control {
...
<div class="kt-card max-w-4xl mx-auto">
    <div class="kt-card-header" id="agreement_settings">
        <h3 class="kt-card-title">Edit Agreement</h3>
    </div>

    @if ($errors->any())
        <div class="bg-destructive/10 border-s-4 border-destructive text-red-700 p-4 mb-4 fade-in" role="alert">
    ...
    <form action="{{ route('agreements.update', $agreement->id) }}" method="POST" id="agreement-form" novalidate class="kt-card-content grid gap-5 md:grid-cols-2 lg:grid-cols-3">
        @csrf
        @method('PUT')
        ...
        <div class="w-full">
            <div class="flex flex-col gap-2.5">
                ...
                <input type="text" name="title" id="title" class="kt-input" value="{{ old('title', $agreement->title) }}" placeholder="Enter agreement title" required>
            </div>
        </div>
        ...
        <div class="w-full">
            <div class="flex flex-col gap-2.5">
                ...
                <input type="text" name="reference_code" id="reference_code" class="kt-input" value="{{ old('reference_code', $agreement->reference_code) }}" placeholder="Enter reference code" required>
            </div>
        </div>
            ...
            <div class="flex flex-col gap-2.5">
                ...
                <select name="agreement_type_id" id="agreement_type_id" class="kt-input" required>
                    ...
                    @foreach($agreementTypes as $type)
                        <option value="{{ $type->id }}" {{ old('agreement_type_id', $agreement->agreement_type_id) == $type->id ? 'selected' : '' }}>{{ $type->type_name }}</option>
                    @endforeach
                </select>
            </div>
        </div>
            ...
            <div class="flex flex-col gap-2.5">
                ...
                <select name="department_id" id="department_id" class="kt-input" required>
                    ...
                    @foreach($departments as $department)
                        <option value="{{ $department->id }}" {{ old('department_id', $agreement->department_id) == $department->id ? 'selected' : '' }}>{{ $department->department_name }}</option>
                    @endforeach
                </select>
            </div>
        </div>
        ...
        <div class="w-full">
            <div class="flex flex-col gap-2.5">
                ...
                <input type="date" name="signing_date" id="signing_date" class="kt-input" value="{{ old('signing_date', $agreement->signing_date?->format('Y-m-d')) }}" required>
            </div>
        </div>
        ...
        <div class="w-full">
            <div class="flex flex-col gap-2.5">
                ...
                <input type="date" name="effective_date" id="effective_date" class="kt-input" value="{{ old('effective_date', $agreement->effective_date?->format('Y-m-d')) }}" required>
            </div>
        </div>
        ...
        <div class="w-full">
            <div class="flex flex-col gap-2.5">
                ...
                <input type="date" name="expiry_date" id="expiry_date" class="kt-input" value="{{ old('expiry_date', $agreement->expiry_date?->format('Y-m-d')) }}" required>
            </div>
        </div>
        ...
        <div class="w-full">
            <div class="flex flex-col gap-2.5">
                ...
                <select name="partner_institutions[]" id="partner_institutions_select" multiple class="tom-select">
                    <option value="">-- No partner institutions --</option>
                    @foreach ($partners as $partner)
                        <option value="{{ $partner->id }}" {{ in_array($partner->id, old('partner_institutions', $agreement->partners->pluck('id')->toArray())) ? 'selected' : '' }}>
        ...
        <div class="w-full md:col-span-2 lg:col-span-3">
            <div class="flex flex-col gap-2.5">
                ...
                <textarea name="description" id="description" rows="5" class="kt-input" placeholder="Enter agreement description" required>{{ old('description', $agreement->description) }}</textarea>
            </div>
        </div>
